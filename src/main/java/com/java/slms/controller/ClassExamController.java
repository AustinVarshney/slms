...
public class ClassExamController
{
    ...
    @Operation(summary = "List classes assigned to an exam type")
    @GetMapping("/exam-type/{examTypeId}")
    public ResponseEntity<RestResponse<List<ClassExamResponseDto>>> getByExamType(
            @PathVariable Long examTypeId,
            @RequestAttribute("schoolId") Long schoolId
    )
    {
        var data = classExamService.getClassesByExamType(examTypeId, schoolId);
        return ResponseEntity.ok(
                RestResponse.<List<ClassExamResponseDto>>builder()
                        .data(data)
                        .message("Found: " + data.size())
