...
public class StaffLeaveAllowanceServiceImpl implements StaffLeaveAllowanceService
{
    ...
    private final com.java.slms.repository.StaffLeaveRecordRepository staffLeaveRecordRepository;
    ...
    public StaffLeaveAllowanceDto getAllowance(Long staffId, Long sessionId, Long schoolId)
    {
        ...
        // Calculate leaves used (approved leaves)
        int leavesUsed = staffLeaveRecordRepository.countApprovedLeaves(staffId, sessionId, schoolId);
        int remainingLeaves = allowance.getAllowedLeaves() - leavesUsed;

        return StaffLeaveAllowanceDto.builder()
                .staffId(allowance.getStaff().getId())
                .sessionId(allowance.getSession().getId())
                .sessionName(allowance.getSession().getName())
                .allowedLeaves(allowance.getAllowedLeaves())
                .leavesUsed(leavesUsed)
                .remainingLeaves(Math.max(0, remainingLeaves)) // Don't go negative
                .build();
    }
    ...
    public List<StaffLeaveAllowanceDto> getAllowancesForSession(Long sessionId, Long schoolId)
    {
        ...
        return allowanceRepo.findAllBySessionIdAndSchoolId(sessionId, schoolId)
                .stream()
                .map(allowance -> {
                    // Calculate leaves used for each staff
                    int leavesUsed = staffLeaveRecordRepository.countApprovedLeaves(
                            allowance.getStaff().getId(), sessionId, schoolId);
                    int remainingLeaves = allowance.getAllowedLeaves() - leavesUsed;
